...
exports.do=function(arg, callback, callback_err, idata){
	...
	if(provider && displayName){
		...
		db.sql("select iduser from m.users where provider=$1 and username ilike $2;", [ provider, displayName ], function(result){
...
function addnew(provider, id, displayName, md5pass, enabled, callback, callback_err, idata){
	var data = idata || {};
	db.sql("insert into m.users (provider, id, username, md5pass, dt, enabled) values ($1, $2, $3, $4, now(), $5) returning iduser, provider, id, username;", [provider, id, displayName, md5pass, enabled], function(result){
		if(result && result.rows && result.rows.length && result.rows[0], result.rows[0].iduser>0){
			if( result.rows[0].id>0 && result.rows[0].provider!=myname ){
				...
				db.sql("update m.users set id=iduser where iduser=$1 returning iduser;", [ result.rows[0].iduser ], function(result){
